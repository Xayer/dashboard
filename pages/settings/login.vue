<template>
	...
	<form v-else ref='form' @submit.stop.prevent="submitForm">
		...
		<label for="password">Password:
			<FormInput v-model="password" name="password" type="password" class="form-field m-b" />
		</label>
...
export default class Login extends Vue {
	...
	submitForm() {
		...
		this.error = '';
		this.$store.dispatch('userSettings/authenticate', { username, password }).catch((error) => {
			this.error = error;
		});
	}
